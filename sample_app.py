...
with tab1:
        ...
        if data:
                ...
                # "すべて選択" チェックボックス
                select_all = st.sidebar.checkbox("全ての圃場を選択")
                
                # チェックボックスの状態に応じてMultiselectウィジェットを制御
                if select_all: #全て選択ボタンが押されている場合
                    if dev_key2:
                        ind_dev_key_stocks = st.sidebar.multiselect(label="圃場ID（必須）",options=dat2.query('生産者 in @dev_key2')[dev_key].unique(),default=dat2.query('生産者 in @dev_key2')[dev_key].unique())
                    else:
                    ...
                    if dev_key2:
                        ind_dev_key_stocks = st.sidebar.multiselect(label="圃場ID（必須）",options=dat2.query('生産者 in @dev_key2')[dev_key].unique())
                    else:
                ...
                make_fig = st.sidebar.checkbox("グラフを出力する（圃場数≦200の場合のみ）")
                ...
                if ind_dev_key_stocks:
                        ...
                        time_course = st.sidebar.slider("調査期間", value=(min_date, max_date), min_value=min_date, max_value=max_date)
...
with tab2:
        ...
        # 地図表示する際の中心座標を指定
        map = folium.Map(location=[43.342009,142.383147], zoom_start=7)
